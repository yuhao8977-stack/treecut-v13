...
CREATE UNIQUE INDEX IF NOT EXISTS idx_assets_media_unique ON assets(media_id);
"""
...
class AssetsManager:
    ...
    def ensure_all_video_assets(self) -> int:
        ...
        with self._connect() as connection:
            rows = connection.execute(
                "SELECT m.id media_id,m.relative_path,s.path source_path FROM media_files m "
    ...
    def finalize_fingerprint(self, media_id: int, absolute_path: str) -> None:
        """Compute the full streaming SHA256 once the file is stable (P1 dedup)."""
        now = time.time()
