...
FUNCTION_KEYWORDS = {
    "伸缩": ["伸缩", "展开", "收缩", "折叠", "抽拉", "变形", "拿出来", "拉出来",
            "收进去", "变长", "加长", "延伸", "坐6个人", "坐10个人", "伸縮"],
    "收纳": ["收纳", "储物", "分类", "薄抽", "上层薄抽", "下层抽屉", "对开门",
            "柜门", "塞"],
    "抽屉": ["抽屉", "薄抽", "深抽", "抽拉", "托底", "三节", "飽抽", "饱抽",
            "放筷子", "放勺子", "活动层板"],
    "隐藏": ["隐藏", "隐形", "无把手", "嵌入式"],
    "插座": ["插座", "充电", "电源"],
    "轨道插座": ["轨道插座", "公牛轨道", "一组轨道", "配置了轨道"],
    "隐藏电器": ["烤箱", "洗碗机", "蒸箱", "冰箱", "电器", "精准开孔",
                 "预留位置", "嵌入"],
    "水吧": ["水吧", "水槽", "水龙头", "吧台", "洗水果", "下凹", "泡茶"],
}
...
SIMPLIFY_MAP = str.maketrans(
    ...
    "縮飽臺際風線軌頭組導鐘業幹圓臺聲課護行測設講製預專關會為電聯與實運買賣",
    "刚开门户车体厂处单对说这来时间里内吗让当过还后从点问题颜色个样开关键盘边"
    "缩饱台际风线轨头组导钟业干圆台声课护行测设讲制预专关会为电联与实运买卖",
)
...
class IndustryEngine:
    ...
    def _classify_content_v2(self, text: str, asr_text: str = "") -> tuple[str, float, list[str], dict]:
        ...
        if case_strong >= 2:
            ...
            # 空镜 + 功能演示标签 → 功能展示
            main_type = "功能展示"
        elif not has_talk and has_prod_show:
            # 空镜/无讲解 + 产品展示信号（空镜/造型/细节/整体展示）→ 产品展示
            main_type = "产品展示"
