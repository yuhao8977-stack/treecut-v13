# -*- coding: utf-8 -*-
"""FRESH_HOLDOUT_BLIND_UI_FIX_REPORT 生成。"""
import os
import shutil
import subprocess
import sys
...
R = []
R.append(f"""# FRESH_HOLDOUT_V1 — Blind Review UI Blocking Fix Report

- **日期**：{NOW} ｜ git `{commit}` ｜ 性质：仅修 Blind Review UI 数据装载/布局；未动 AI 预测/Bundle/题目/manifest
...
""")
report = os.path.join(DOCS, "FRESH_HOLDOUT_BLIND_UI_FIX_REPORT.md")
with open(report, "w", encoding="utf-8") as f:
    f.write("\n".join(R))
desktop = os.path.join(os.environ.get("USERPROFILE", r"C:\Users\admin"), "Desktop")
shutil.copy2(report, os.path.join(desktop, "FRESH_HOLDOUT_BLIND_UI_FIX_REPORT.md"))
print("报告 ->", report)
