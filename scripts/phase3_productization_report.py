# -*- coding: utf-8 -*-
"""Phase 3 PART F — REVIEW_SYSTEM_PRODUCTIZATION_REPORT.md 生成。"""
import json
import os
import shutil
import subprocess
...
summ = json.load(open(os.path.join(DATA_ROOT, "PHASE3_FINALIZATION_SUMMARY.json"), encoding="utf-8"))
man = json.load(open(os.path.join(DATA_ROOT, "CALIBRATION_CORPUS_V2_MANIFEST.json"), encoding="utf-8"))
cov = json.load(open(os.path.join(DATA_ROOT, "COVERAGE_MATRIX_V3.json"), encoding="utf-8"))

R = []
R.append(f"""# TreeCut Review System Productization — 审核系统产品化报告

- **日期**：{NOW} ｜ 仓库 `{commit}` ｜ 词典：ANNOTATION_DICTIONARY_V2_1
...
- COVERAGE_MATRIX_V3：GOOD {cov['state_counts'].get('GOOD',0)} / MEDIUM {cov['state_counts'].get('MEDIUM',0)} / LOW {cov['state_counts'].get('LOW',0)}；**长尾改善有限（素材库先天偏科 工厂×岛台×岩板 98%+），诚实报告**
...
| 14 | 最新 Coverage | V3：GOOD {cov['state_counts'].get('GOOD',0)} / MEDIUM {cov['state_counts'].get('MEDIUM',0)} / LOW {cov['state_counts'].get('LOW',0)}（`COVERAGE_MATRIX_V3.json`） |
| 15 | pytest | **121 passed**（新增：widget leak load100、mandatory trace 100、playback 3 项、生命周期 20 次、单实例、responsive 4 尺寸、任务统计） |
| 16 | git commit | `{commit}`（main） |
...
""")
report = os.path.join(DOCS, "REVIEW_SYSTEM_PRODUCTIZATION_REPORT.md")
with open(report, "w", encoding="utf-8") as f:
    f.write("\n".join(R))
desktop = os.path.join(os.environ.get("USERPROFILE", r"C:\Users\admin"), "Desktop")
shutil.copy2(report, os.path.join(desktop, "REVIEW_SYSTEM_PRODUCTIZATION_REPORT.md"))
print("报告 ->", report)
print("已复制桌面")
