# -*- coding: utf-8 -*-
"""Phase 3 A8 — PHASE3_HUMAN_DATA_FINALIZATION_REPORT.md 生成。"""
import json
import os
import shutil
import sqlite3
...
R = []
R.append(f"""# Phase 3 Human Data Finalization — 94 条人工数据结算报告
...
**状态分布**：GOOD {cov['state_counts'].get('GOOD',0)} / MEDIUM {cov['state_counts'].get('MEDIUM',0)} / LOW {cov['state_counts'].get('LOW',0)} / EMPTY {cov['state_counts'].get('EMPTY',0)}（{cov['total_combos']} 组合，人口 333 段）
...
""")
report = os.path.join(DOCS, "PHASE3_HUMAN_DATA_FINALIZATION_REPORT.md")
with open(report, "w", encoding="utf-8") as f:
    f.write("\n".join(R))
desktop = os.path.join(os.environ.get("USERPROFILE", r"C:\Users\admin"), "Desktop")
shutil.copy2(report, os.path.join(desktop, "PHASE3_HUMAN_DATA_FINALIZATION_REPORT.md"))
print("报告 ->", report)
print("已复制桌面")
