# -*- coding: utf-8 -*-
"""Phase 3 — Review UI Architecture Audit（只读审计报告生成，不修改任何 UI/数据）。"""
import os
import shutil
import sqlite3
import subprocess
...
R = []
R.append(f"""# TreeCut Review UI Architecture Audit — 人工审核窗口架构审计

- **日期**：{NOW} ｜ 仓库 `{commit}` ｜ 性质：**只读审计，未修改任何 UI/数据库/审核数据/模型**
...
""")
report = os.path.join(DOCS, "REVIEW_UI_ARCHITECTURE_AUDIT.md")
with open(report, "w", encoding="utf-8") as f:
    f.write("\n".join(R))

desktop = os.path.join(os.environ.get("USERPROFILE", r"C:\Users\admin"), "Desktop")
shutil.copy2(report, os.path.join(desktop, "REVIEW_UI_ARCHITECTURE_AUDIT.md"))
print("报告 ->", report)
print("已复制 ->", os.path.join(desktop, "REVIEW_UI_ARCHITECTURE_AUDIT.md"))
